...
- **`tier` is an entitlement, not a plan identity.** `useTier().tier` is `'free'` for anonymous visitors — correct for feature gates, wrong for anything that badges or names a plan. Plan-facing UI reads `useTier().hasAccount` too and claims nothing when there is no account (TE-44; see `DECISIONS.md`).
- **No plan action fires before the tier is read.** `useTier().tierLoading` is true between sign-in and the first Firestore snapshot, during which `tier` still reads `'free'` for a subscriber. Checkout, portal hand-offs and downgrades all wait for it; feature gates deliberately do not (TE-45; see `DECISIONS.md`). Anything offering a plan also passes the _clicked_ plan down — never re-derives it — and clamps its selection to the options it renders.
...
- **Upgrades bill now, downgrades wait.** pro→builder is immediate and prorated (`always_invoice`: Stripe charges the net difference on the spot, billing anchor unchanged); builder→pro is deferred to the period end (`schedule_at_period_end`), because that time is already paid for. Both come from the **portal configuration**, applied by `npm run stripe:configure-portal` — not from anything the session sends. A deployment where that has never run is mis-billing (TE-47; see `DECISIONS.md`).
- **A hand-off to Stripe is not a completed journey.** `POST /api/portal` takes a `targetTier` and deep-links via `flow_data` to the flow the user asked for; a bare session opening the portal homepage is the degraded fallback, never the plan. `targetTier` is a routing hint — the tier is always re-read server-side and the flow discarded unless it describes a real transition.
- **A scheduled switch is separate state from the current tier.** A period-end downgrade leaves the current price live, so `customer.subscription.updated` looks like a no-op; `pendingTier`/`pendingTierDate` are read off the subscription schedule and written on every sync, `null` included. `tier` never moves early.
- **Downgrades to free happen at period end**, driven by webhook events (`customer.subscription.deleted` → free; `customer.subscription.updated` → status/plan mirror). `invoice.payment_failed` warns (`past_due` + alert) but never drops tier.
...
| `users/{uid}.pendingTier` / `.pendingTierDate`                           | scheduled period-end plan switch (TE-47) — server-written, never an entitlement check                  |
