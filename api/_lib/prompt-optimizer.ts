...
export async function runSelfImprovement(db: any, force = false): Promise<void> {
  ...
  // 3b. Fetch engagement counters (idea_stats/{date}_{ideaId}) for the same ideas.
  // Doc-id gets — no query index needed. Fail-open: engagement block is optional.
  let engagementContext = '';
  try {
    const statSnaps = await Promise.all(
      recentIdeas.map((i) => db.collection('idea_stats').doc(`${i.dateGenerated}_${i.id}`).get())
    );
    const engaged: string[] = [];
    const ignored: string[] = [];
    for (let i = 0; i < recentIdeas.length; i++) {
      const snap = statSnaps[i];
      if (!snap.exists) continue;
      const s = snap.data() || {};
      const impressions = s.impression || 0;
      if (impressions < 10) continue; // too little data to judge
      const expands = s.expand || 0;
      const saves = s.save || 0;
      const expandRate = Math.round((expands / impressions) * 100);
      const saveRate = Math.round((saves / impressions) * 100);
      const line = `"${recentIdeas[i].headline}" — ${impressions} views, ${expandRate}% expanded, ${saveRate}% saved`;
      if (expands === 0 && saves === 0) {
        ignored.push(line);
      } else if (expandRate >= 20 || saves > 0) {
        engaged.push(line);
      }
    }
    if (engaged.length > 0 || ignored.length > 0) {
      engagementContext =
        (engaged.length > 0
          ? `\nHIGH-ENGAGEMENT IDEAS (users actively read/saved these — generate more like them):\n${engaged.slice(0, 10).join('\n')}\n`
          : '') +
        (ignored.length > 0
          ? `\nIGNORED IDEAS (many views, zero interest — avoid these patterns):\n${ignored.slice(0, 10).join('\n')}\n`
          : '');
    }
  } catch (err) {
    console.error('[prompt-optimizer] Error fetching engagement stats:', err);
  }
