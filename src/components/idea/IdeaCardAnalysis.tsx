...
import { LockedAnalysisSection } from './LockedAnalysisSection';
...
export const IdeaCardAnalysis: React.FC<IdeaCardAnalysisProps> = ({
  idea,
  tier = 'pro',
  onUpgradeClick,
}) => {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid md:grid-cols-2 gap-3 items-start">
        ...
        {isFree ? (
          <LockedAnalysisSection label="Unfair Advantage" onUnlockClick={onUpgradeClick}>
            <div className="space-y-2">
              ...
              <p className="text-xs text-zinc-400 leading-relaxed bg-zinc-800/30 p-3 rounded-lg border border-zinc-800/50">
                {idea.unfairAdvantage || (
                  <span className="text-zinc-600 italic">Not assessed for this idea</span>
                )}
              </p>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-amber-500 relative group/help">
              ...
              <div className="absolute bottom-full mb-2 left-0 w-52 p-2.5 bg-zinc-800 text-xs text-zinc-300 rounded-lg opacity-0 invisible group-hover/help:opacity-100 group-hover/help:visible transition-all z-50 border border-zinc-700 shadow-xl pointer-events-none leading-relaxed">
                Proprietary edge: data moat, patents, network effects, or unique distribution
                channels.
              </div>
            </div>
            <p className="text-xs text-zinc-400 leading-relaxed bg-zinc-800/30 p-3 rounded-lg border border-zinc-800/50">
      ...
      {isFree ? (
        <LockedAnalysisSection label="Revenue Model" onUnlockClick={onUpgradeClick}>
          <div className="space-y-2">
            ...
            <p className="text-xs text-zinc-400 leading-relaxed bg-zinc-800/30 p-3 rounded-lg border border-zinc-800/50 font-mono">
              {idea.revenueSkeleton || (
                <span className="font-sans italic text-zinc-600">No revenue model specified</span>
              )}
            </p>
          </div>
        </LockedAnalysisSection>
      ...
      {(idea.marketSize || idea.competitorLandscape || idea.regulatoryFlags) && (
        <>
          {isFree ? (
            <LockedAnalysisSection label="Market Dynamics" onUnlockClick={onUpgradeClick}>
