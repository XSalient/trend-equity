        ...
        <div className="space-y-6 max-w-sm mx-auto">
          ...
          <div className="flex flex-col gap-4">
            ...
            >
              ...
              <div className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${dailyOn ? 'bg-emerald-500' : 'bg-zinc-700'}`}>
                <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-lg transition-all duration-200 ${dailyOn ? 'right-0.5' : 'left-0.5'}`} />
              </div>
            </button>
            ...
            >
              ...
              <div className={`w-10 h-5 rounded-full relative transition-colors duration-200 ${radarOn ? 'bg-emerald-500' : 'bg-zinc-700'}`}>
                <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-lg transition-all duration-200 ${radarOn ? 'right-0.5' : 'left-0.5'}`} />
              </div>
            </button>
          </div>
          ...
          {user ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 p-3 bg-zinc-800/60 border border-zinc-700/50 rounded-xl text-left">
                ...
                <p className="text-xs text-zinc-400">Sign in to save your preferences and receive digests.</p>
              </div>
            </div>
          )}

          <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest">
            {user?.email ? `Digests are sent to ${user.email}` : 'Sign in to set your delivery email'}
          </p>
        </div>
      </div>
    </div>
