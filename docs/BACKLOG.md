...
| TE-59 | **Defect (ships alone)** — free-tier Evidence "Upgrade →" CTA is unreachable: `pointer-events-none` on the tooltip container blocks the click, and hover-only reveal hides it entirely on touch (`IdeaCard.tsx:277–298`)   | shipped | Claude | S      |
...
**TE-59 as shipped (2026-07-31).** The reported symptom was one of two independent breaks in the same path — the Change Impact pass found the second. States enumerated for the gated Evidence control:

| State                       | Before                                                   | After                          |
| --------------------------- | -------------------------------------------------------- | ------------------------------ |
| Free, desktop hover + click | tooltip shows; `pointer-events-none` swallows the click  | control itself opens upgrade   |
| Free, touch                 | `group-hover`-only — CTA never appears at all            | tap opens upgrade              |
| Free, keyboard              | outer button `disabled` — not focusable, CTA unreachable | focusable, Enter opens upgrade |
| Free, no `onUpgrade` passed | silently dead control                                    | renders inert (locked style)   |
| Pro/Builder                 | gathers or expands evidence                              | unchanged                      |

The second break: `SavedIdeas.tsx` rendered `IdeaCard` **twice without `onUpgrade`**, so a free user's saved ideas had no upgrade path even with the tooltip fixed — the same missing-reader class as `checkIpRateLimit` (TE-02) and `checkoutTier` (TE-45). Both sites now pass the existing `onUpgradeNeeded` prop. `WeeklyBest.tsx` also omits `onUpgrade` and is deliberately left alone: it never fetches for free (`tier !== 'free'` guard), so no free-variant card renders there — revisit if that gate moves. No `users/{uid}` field added, so no `firestore.rules` change; no non-code deploy step. Guarded by `tests/unit/components/IdeaCardEvidenceGate.test.tsx`, which asserts the journey (a free user reaches the upgrade path) rather than the markup.
...
| TE-59 | Free-tier Evidence upgrade CTA made reachable: the gated control is the CTA (click, tap and keyboard), plus the missing `onUpgrade` at both Saved-ideas render sites | 2026-07-31 | (this commit)             |
