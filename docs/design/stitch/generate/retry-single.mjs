...
try {
  ...
  // image fetch is best-effort — keep the screen record even without it
}
const rec = {
  name: '3-analyze-my-idea',
  id: screen?.data?.name ?? screen?.data?.id,
  image,
  secs: 'retry',
};
